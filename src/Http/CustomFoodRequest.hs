...
getCustomFoods
  ...
  -> m [CustomFoodPreview]
getCustomFoods ps = fmap responseBody request
 where
  path    = (/: "custom-food")
  request = reqUnsecure GET path NoReqBody jsonResponse $ params ps
  params (Description d, PageLimit { page, limit }) =
    "description" =: d <> "page" =: page <> "limit" =: limit
